...
for t=0:timestep:timeend
   ...
   % --- 7. Visualization ---
    ...
    % A. 绘制 Voronoi (背景)
    % 稍微调低透明度，让无人机更明显
    plot(p, 'alpha', 0.05, 'EdgeColor', [0.8 0.8 0.8]); 
    
    % B. 绘制 Evaders (Birds)
    if ~isempty(pos_evaders)
        % 用红色小三角代表鸟，方向可以简单朝向速度方向或者随机
        scatter3(pos_evaders(:,1), pos_evaders(:,2), pos_evaders(:,3), ...
            80, 'r', 'filled', '^', 'MarkerEdgeColor', 'r');
            
        % 绘制距离保护罩 (Wireframe 模式比较好看，不会遮挡无人机)
        [sx, sy, sz] = sphere(12);
    ...
    % C. 绘制 Pursuers (Quadrotors / UAVs)
    % 替代原来的 scatter3，调用自定义函数画无人机
    drone_size = 0.5; % 无人机尺寸
    for k = 1:size(pos_pursuers, 1)
        draw_drone(pos_pursuers(k, :), drone_size);
    end
    
    % D. 添加 Legend (图例)
    % 技巧：创建两个不可见的 "Dummy" 图形对象，专门用于显示图例
    % 这样可以避免图例里出现奇怪的 Voronoi 线条或 sphere 面
    ...
    h_legend_min  = plot3(NaN, NaN, NaN, 'r-', 'LineWidth', 1);
    h_legend_max  = plot3(NaN, NaN, NaN, 'g-', 'LineWidth', 1);
    
    legend([h_legend_uav, h_legend_bird, h_legend_min, h_legend_max], ...
           {'UAV Swarm', 'Bird Target', 'Min Dist (Safety)', 'Max Dist (Connect)'}, ...
