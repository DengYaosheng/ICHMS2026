...
%     end
    ...
    % =========================================================
    % === 关键修复：在所有图画完之后，强制锁定坐标轴 ===
    % =========================================================
    
    % 1. 强制设定范围 [-10, 10]
    axis([lbound(1) ubound(1) lbound(2) ubound(2) lbound(3) ubound(3)]);
    
    % 2. 强制锁定数据比例 (DataAspectRatio)
    % 保证 1米的长度在 x,y,z 轴上看起来一样长
    daspect([1 1 1]); 
    
    % 3. 强制锁定绘图框比例 (PlotBoxAspectRatio)
    % 保证绘图区域是一个正立方体，而不是扁盒子
    pbaspect([1 1 1]); 
    
    % 4. 开启透视和网格
    view(3); 
    grid on; 
    box on; % 把立方体的边框画出来，视觉上更稳定
