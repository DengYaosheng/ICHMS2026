...
%     pos_all = [[pos_evaders; pos_pursuers], [zeros(n_e,1); ones(n_p, 1)]]; % 第三列为1为pursuer，否则为evader
    ...
    % 使用空数组 [] 表示使用默认大小，'r' 表示红色
scatter3(pos_evaders(:,1), pos_evaders(:,2), pos_evaders(:,3), [], 'r', 'filled', 'Marker', 'd');
% 语法: scatter3(X, Y, Z, Size, Color, 'filled', 'Marker', Shape)
scatter3(pos_pursuers(:,1), pos_pursuers(:,2), pos_pursuers(:,3), [], 'k', 'filled', 'Marker', 'd');    title(t);
    hold off;
    drawnow;
end
